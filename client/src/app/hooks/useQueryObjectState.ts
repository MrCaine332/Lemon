...
function findOtherParams<Type>(keys: string[], newSearchParams: URLSearchParams) {
...
function findAndRewriteParams<Type>(params: Type, newSearchParams: URLSearchParams, serialize: (value: any) => string) {
	for (let key in params) {
		if (params[key])
			newSearchParams.set(key, serialize(params[key]))
	}
}
...
export function useQueryObjectState<Type extends object>({
	...
	const updateValue = useEvent((newValue: Type | ((prevValue: Type) => Type)) => {
