...
import {RegistrationForm} from "@modules/header/components/account/account-auth/AccountAuth";
import React, {ChangeEvent, FormEvent, MouseEvent, useState} from "react";
import {Input} from "@components/ui/input";
import {Button} from "@components/ui/button";
import Icons from "@components/ui/icons";
import {registration} from "@app/http/user-api-calls";
...
const REGISTRATION_FORM = [
	...
	{ type: 'text', placeholder: 'Username', field: 'username', autocomplete: 'username' },
	{ type: 'password', placeholder: 'Password', field: 'password', autocomplete: 'new-password' },
	{ type: 'password', placeholder: 'Confirm Password', field: 'passwordConfirmation', autocomplete: 'new-password' },
]

export const AccountRegistrationForm = ({ setFormType, onRegistration }: RegistrationForm) => {
	...
	const onInputChange = (e: ChangeEvent<HTMLInputElement>, field: keyof RegistrationCredentials) => {
		setCredentials(prev => ({
			...prev,
	...
	const onSubmit = async (e: FormEvent) => {
		...
		try {
			...
			const mappedErrors = mapErrors<RegistrationApiError>(err, defaultErrorMessage)
			...
			<form className={styles.authForm} onSubmit={onSubmit}>
				{
					REGISTRATION_FORM.map((input, index) => (
						<Input key={index}
						       ...
						       onChange={(e) => onInputChange(e, input.field as keyof RegistrationCredentials)}
